...
**入口**
- 仅管理员可见：顶栏在「写文章」左侧渲染「用户管理」按钮，跳 `/admin.html`
- 普通用户访问 `/admin.html` → admin.js 检测 `!user.isAdmin` → 跳 `/list.html`

**布局**
```
┌─────────────────────────────────────────────────────────────┐
│ [Logo] [全部 标签1 ...]   [用户管理] [写文章] [sysadmin▾] [登出] │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   管理后台                                                  │
│   用户管理                                                  │
...
│   │ 用户ID │ 用户名   │ 昵称  │ 注册时间   │ 操作      │          │
│   ├──────┼─────────┼──────┼──────────┼──────────┤          │
│   │ 2    │ alice   │ Alice│ 2026/5/1 │ [删除]   │          │
│   │ 3    │ bob     │ Bob  │ 2026/5/2 │ [删除]   │          │
│   └──────┴─────────┴──────┴──────────┴──────────┘          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
**数据加载**
1. `GET /api/v1/admin/users` → 渲染用户表格
2. 删除按钮点击 → `confirm('确定删除用户 xxx 及其发布的全部文章？此操作不可恢复。')`
3. 确认后 `DELETE /api/v1/admin/users/:id` → 成功后 toast + 刷新列表

**样式**
- 标题区复用 `.hero-kicker` + `.hero-title`
- 表格使用 `.card` 容器，行用 `.admin-row`，表头 `.admin-header`
- 删除按钮使用 `.btn-danger`（红色系）

**顶栏改动（navbar.js）**
- 调用 `/auth/me` 获取用户后，若 `user.isAdmin === true`，在「写文章」左侧插入「用户管理」按钮

**登录重定向（login.js）**
- 登录成功后检查 `data.isAdmin`：管理员 → `/admin.html`；普通用户 → `redirect` 参数页或 `/list.html`
...
| 2001 | （静默） | 跳登录页 |
| 2002 | 无权操作（非管理员访问后台 / 非作者编辑文章） | 退回列表 / 跳转首页 |
...
- [ ] sysadmin / admin111 登录 → 顶栏显示「用户管理」入口，跳转 `/admin.html`
- [ ] 管理员在 admin 页查看用户列表 → 显示所有注册用户
- [ ] 管理员点击删除 → 二次确认后用户消失，该用户文章也被删除
- [ ] 普通用户直接访问 `/admin.html` → 自动跳 `/list.html`
- [ ] 普通用户顶栏无「用户管理」按钮
